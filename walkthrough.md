...
- [**Virtual Private Cloud Networking on PfSense**](#virtual-private-cloud-networking-on-pfsense)
  - [**SCENARIO**](#scenario)
  - [**IMPLEMENTATION**](#implementation)
    - [**STEP-1:** Creating Virtual Interfaces](#step-1-creating-virtual-interfaces)
    - [**STEP-2:** Creating Virtual Machines](#step-2-creating-virtual-machines)
    - [**STEP-3:**  Configuring the IGW Router](#step-3--configuring-the-igw-router)
    - [**STEP-4:**  Configuring DHCP on IGW](#step-4--configuring-dhcp-on-igw)
    - [**STEP-5:**  Configuring the AV-1 Router](#step-5--configuring-the-av-1-router)
    - [**STEP-6:**  Configuring DHCP on AV1](#step-6--configuring-dhcp-on-av1)
    - [**STEP-7:**  Configuring Routing on IGW](#step-7--configuring-routing-on-igw)
    - [**STEP-8:** Configuring Firewall on IGW](#step-8-configuring-firewall-on-igw)
    - [**STEP-9:** Routing on Availability Zone - 1](#step-9-routing-on-availability-zone---1)
    - [**STEP-10:** Firewall on Availability Zone - 1](#step-10-firewall-on-availability-zone---1)
    - [**STEP-11:** DNS Configuration  on IGW](#step-11-dns-configuration--on-igw)
    - [**STEP-12:** DNS on Availability Zone - 1](#step-12-dns-on-availability-zone---1)
...
To solve the above task, we are going to have multiple network interfaces as shown in the figure below. The IGW pfSense will be the primary pfsense router since it handles perimeter firewall, DNS and the routing of packets.

![Solution](Images/CCDC_Practice_Scenario_1.png)  

### **STEP-1:** Creating Virtual Interfaces

To implement the above network map, the first step would be create *virtual networks* on XOA (Baremetal Hypervisor) or any hypervisor so that we can attach machines to a particular network. They are named as follows in this case:
...
Create 3 PfSense virtual machines and attach the network interfaces as follows to create availability zones and create multiple private sub networks to isolate devices connected to those networks from being directly accessible to the outside world.

**VIrtual Machine -1 (IGW):** The main Internet Gateway router or it can also be termed as the edge router. This is where the traffic comes in from outside and the traffic from internal network goes out. The availability zones would be connected to the IGW_Gateway which makes the availability zones directly connected networks to the IGW. We need two interfaces on this router. The first network interface (WAN) would be a bridged network while the other one would be a virtual interface which is a LAN network that the availability zones would be connected to. We call this the IGW_GATEWAY.
...
- Once pfsense is installed, open the console and select option1 to  and assign the WAN and LAN interfaces to their physical interfaces respectively. When a prompt asks you to setup VLANs, answer NO.  Note that the virtual interfaces are also considered a physical interface as shown in the image below.

![Assign Interfaces](Images/2.png)

- Once you assign interfaces, the WAN would  get an IP address from the DHCP server since the WAN interface is connected in bridged mode to the school. 
...
- Configure WAN on the IGW pfSense using DHCP to get an IP from the school. We would get an IP from the Cyber Range LAN which is a 192.168.0.0/21 network.
  
![WAN](Images/3.png)

- Configure LAN (IGW Gateway) - Setup a static IPv4 with IPv6 disabled. The network it is on in this case is 10.5.5.0/24. The IP of the interface is 10.5.5.1 and it is also the gateway. As a default networking rule, the IP address of the network interface would be the gateway. The IPv4  upstream gateway should be set to none because enabling IPv4 upstream gateway would treat this as a WAN type interface which is not what the IGW_Gateway interface is intended to in this case.
  
 ![LAN](Images/4.png)

### **STEP-4:**  Configuring DHCP on IGW

- Configure DHCP on the LAN interface by navigating to ***Services > DHCP*** Server. Enable the DHCP on the interface and the available range for DHCP is 10.5.5.10 - 10.5.5.240 (The range is auto-populated by default). A rule of thumb is that for any network interface, the gateway would be the IP address of the network interface itself and in this case the IP address is 10.5.5.1. Enter this as the gateway when configuring the DHCP.
...
- Follow the same process as we did for IGW Router - Assign interfaces (WAN, LAN and OPT1). Once the interfaces are assigned, the IP address of the WAN would be 10.5.5.10 since that is the first available address for IGW_Gateway network.
- Since our local machine is on a different network than the WAN of AV-1, we would need another device which is on the same network to access the Web Configuration page of pfsense and hence we connect another machine to the IGW_Gateway network and access the web configuration page for AV-1.
- Navigate to 10.5.5.10 in the web browser of the machine connected to IGW_Gateway network.
- Configure the LAN as AV1_PUB and assign it an IP address of 10.1.1.1. The CIDR would be 10.1.1.0/24.
- The gateway would be 10.1.1.1
...
- On AV1_PRIV, add a rule that passes incoming data from AV1_Pub network since we want the private subnet to be isolated from the outside. The Private subnet should only be able to communicate with the web servers connected to the public subnet. You can only allow specific ports if you already have services up and running. 
...
- In order to configure DNS for our networks, we would need DNS servers and hence we are using external DNS servers and the in-house DNS server to resolve hostnames.
...
- To solve for DNS on this zone, the idea is to forward the DNS requests to the IGW_Gateway network and the IGW router will handle the resolution of DNS requests.
- List the IP address of the IGW_Gateway interface which is 10.5.5.1 in the ***System > General Setup > DNS Server Settings*** field
...
The above guide helps replicate a Virtual Private Cloud like networking architecture using PfSense. The firewall rules and IP addresses can be modified as deemed fit. If you have multiple static IP addresses from your internet service provider, you can configure Virtual IPs to host multiple web services.
